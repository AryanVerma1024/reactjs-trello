...
export default function Listing() {
    ...
    const [lists,setLists] = useState({
        '1671110904481': {title:'To Do',
                    items:{
                        '1671110940450':'Helpdesk Call BB999',
        ...
        '1671110904482': {title:'Development',
                    items:{
                        '1671110940452':'Helpdesk Call CC999',
        ...
        '1671110904483': {title:'Testing',
                    items:{
                        '1671110940454':'Helpdesk Call DD999'
                    },
                    check: false
                } ,
        '1671110904484': {title:'Done',
                    items:{
                        '1671110940455':'Helpdesk Call GG999',
    ...
    <div className='listing'>
        <DragDropContext onDragEnd={onDragEnd}>
            {Object.entries(lists).map(([listID, list]) =>
                <Droppable droppableId={listID}>
                    { provided => 
                        <div className='maincontent' key={listID} ref={provided.innerRef} {...provided.droppableProps}>
                            <div className='listitle'>
                                <h3>{list.title}</h3>
                                <span className='material-icons'></span>
                            </div>
                            <div className='listname'>
                                {Object.entries(list.items).map(([itemID, itemContent]) =>
                                    <Draggable key={itemID} draggableId={itemID} index={parseInt(itemID.slice(-1))}>
                                        { provided =>
                                            <p key={itemID} {...provided.draggableProps} {...provided.dragHandleProps} ref={provided.innerRef}>
                                                {itemContent}
                                            </p>
                                        }
                                    </Draggable>
                            ...
                            {list.check ?
                                <form className='cardname' onSubmit={(e) => addItemToList(e, listID)}>
                                    <input autoFocus type='text' placeholder='Enter a title for this card...' onChange={(e) => setNewItemContent(e.target.value)} />
        ...
        {showNewListForm ?
            <div className='newlist' style={{background: 'white'}}>
                <form className='cardname' onSubmit={e => addNewList(e)}>
                    <input autoFocus type='text' placeholder='Enter list title...' onChange={e => setnewListTitle(e.target.value)} />
                    <div className='addcardinput'>
                        <button>
                            <p>Add List</p>
                        </button>
